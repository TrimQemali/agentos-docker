...
- `app.settings.default_model()` returns `OpenAILike(id="glm-4.7", base_url=ZAI_BASE_URL)` — z.ai's GLM over its OpenAI-compatible Coding Plan endpoint, reading `ZAI_API_KEY` (not `OPENAI_API_KEY`). Bump the model, base URL, or provider in one place. Knowledge-base embeddings (`db/session.py`) are unaffected by this swap and still read `OPENAI_API_KEY` — the Coding Plan endpoint is coding-scenario only and has no embeddings API, so knowledge ingestion/search stays unconfigured until a key with embeddings access is set.
